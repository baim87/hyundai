// src/components/Header/HeaderVehicleDetail.js
'use client';
...
// Accept pageData, vehicleSlug, subNavRef, and isSubNavSticky as props
export default function HeaderVehicleDetail({ pageData, vehicleSlug, subNavRef, isSubNavSticky }) {
    ...
    return (
        <header className="header_section hyundai_header bg-black sticky_header clearfix main-site-header">
                        ...
                        <div className="col-lg-6 d-none d-lg-block"> {/* Desktop Main Menu */}
                            <nav className="main_menu clearfix">
                                <ul className="ul_li_center clearfix">
                                    ...
                                    <li className={`menu_item_has_child ${activeSubmenu === 'services' || activeSubmenu === 'layanan' ? 'menu-open' : ''}`} ref={servicesSubmenuContainerRef}>
                                        <a href="#!" onClick={(e) => handleDesktopMenuToggle('sub', 'services', e)}>Services</a>
                                        <ul className="submenu" style={{ display: activeSubmenu === 'services' || activeSubmenu === 'layanan' ? 'block' : 'none' }}>
                                            <li className={`menu_item_has_child ${activeSubmenu === 'layanan' ? 'menu-open' : ''}`}>
                                                <a href="#!" onClick={(e) => handleDesktopMenuToggle('sub', 'layanan', e)}>Layanan</a>
                                                <ul className="submenu" style={{ display: activeSubmenu === 'layanan' ? 'block' : 'none' }}>
                                                    {servicesLayananSubmenu.map(item => (
                                                        <li key={item.title}><Link href={item.href} onClick={() => setActiveSubmenu(null)}>{item.title}</Link></li>
                                                    ))}
                                                </ul>
                                            </li>
                                            <li><Link href="/call-center" onClick={() => setActiveSubmenu(null)}>Call Center</Link></li>
                                            <li><Link href="/e-brochure" onClick={() => setActiveSubmenu(null)}>e-Brochure</Link></li>
                                        </ul>
                                    </li>
            ...
            <div className="">
                {pageData && pageData.subNavigation && pageData.subNavigation.length > 0 && vehicleSlug && (
                    <nav
                        ref={subNavRef}
                        className={`navbar navbar-expand-sm vehicle-sub-navigation ${isSubNavSticky ? 'is-sticky-subnav' : ''}`}
                        style={isSubNavSticky ? { top: `${document.querySelector('.main-site-header')?.offsetHeight || 60}px` } : {}}
                    >
                        <div className="container d-flex justify-content-between align-items-center">
                            {/* Left - Brand/Title */}
                            <Link href={`/vehicle/${vehicleSlug}`} className="navbar-brand text-white vehicle-subnav-title fs-6 me-auto">
                                {pageData.year} {pageData.vehicleName}
                            </Link>

                            {/* Center - Navigation links (collapse on small screens) */}
                            <button
                                className="navbar-toggler d-sm-none"
                                type="button"
                                data-bs-toggle="collapse"
                                data-bs-target={`#vehicleSubNav-${vehicleSlug}`}
                                aria-controls={`vehicleSubNav-${vehicleSlug}`}
                                aria-expanded="false"
                            ...
                            <div className="collapse navbar-collapse justify-content-center" id={`vehicleSubNav-${vehicleSlug}`}>
                                <ul className="navbar-nav">
                                    {pageData.subNavigation.map(navItem => (
                                        <li className="nav-item" key={navItem.id}>
                                            <a className="nav-link" href={navItem.href}>{navItem.label}</a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            ...
                            {/* Right - Buttons */}
                            {pageData.mainHeaderActions && (
                                <div className="d-none d-sm-flex align-items-center subnav-actions ms-auto">
                                    <Link href={pageData.mainHeaderActions.buildPriceLink || '#!'} className="btn btn-light btn-sm bg-opacity-10">
                                        e-Brochure
                                    </Link>
                                </div>
                            )}
